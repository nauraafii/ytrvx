...
MODULE_TEMPLATE_DIR="revanced-kernelsu"
CWD=$(pwd)
TEMP_DIR="temp"
BIN_DIR="bin"
BUILD_DIR="build"
...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		module_prop \
			...
			"${app_name} ${args[rv_brand]} KernelSU module" \
			"https://raw.githubusercontent.com/${GITHUB_REPOSITORY-}/update/${upj}" \
			"$base_template"

		local module_output="${app_name_l}-${rv_brand_f}-kernelsu-v${version_f}-${arch_f}.zip"
...
description=${4}" >"${6}/module.prop"
	...
	if [ "$ENABLE_KERNELSU_UPDATE" = true ]; then echo "updateJson=${5}" >>"${6}/module.prop"; fi
}
